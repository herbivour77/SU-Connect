...
const { withCategories } = require('./contacts');

const COLUMNS = ['contactId', 'fullName', 'categories', 'gender', 'email', 'phone', 'address1', 'address2', 'address3', 'notes', 'archived', 'createdAt'];
const HEADERS = ['Contact ID', 'Full Name', 'Category', 'Gender', 'Email', 'Phone', 'Address 1', 'Address 2', 'Address 3', 'Notes', 'Archived', 'Created At'];
...
exports.handler = async (event) => {
  ...
  let contacts = (await Promise.all(blobs.map((b) => store.get(b.key, { type: 'json' })))).filter(Boolean).map(withCategories);
  ...
  const rows = contacts.map((c) => [
    c.contactId, c.fullName, (c.categories || []).join('; '), c.gender || '', c.email || '', c.phone || '',
