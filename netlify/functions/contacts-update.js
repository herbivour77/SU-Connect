const { contactsStore } = require('./_lib/stores');
...
exports.handler = async (event) => {
  ...
  if (action === 'edit') {
    ...
    const allowed = ['fullName', 'email', 'phone', 'category', 'notes'];
    allowed.forEach((f) => {
      if (f in updates) contact[f] = updates[f];
    });
  ...
  if (action === 'merge') {
    ...
    ['email', 'phone', 'category', 'notes'].forEach((f) => {
