...
const { normalizeGender, normalizeCategories } = require('./contacts');
...
//
// `columnMap` (optional) is a { fullName, email, phone, address1,
// address2, address3, category, gender, notes } object of ACTUAL column
// headers from the uploaded file, chosen by the person via the mapping
// step in the UI before import runs. When present it's used verbatim —
// no guessing. This exists because guessing header names (e.g. "Full
// Name" vs "Name" vs "Full Name (English)" vs something else entirely)
// is inherently fragile against real-world spreadsheets; asking the
// person once is the only fix that reliably works for every file. If no
// columnMap is sent (e.g. an older client, or a programmatic caller), we
// fall back to matching a handful of common header spellings, same as
// before.
//
// A contact can have more than one category (see contacts.js). If the
// mapped/guessed Category column contains "donor, volunteer" in a single
// cell, that becomes two categories, same as typing both into the tag
// picker in the Add Contact form would.
function normalizeRow(raw, columnMap) {
  ...
  return {
    ...
    address1: get('address1', 'address 1', 'address1', 'address', 'street address') || null,
    address2: get('address2', 'address 2', 'address2') || null,
    address3: get('address3', 'address 3', 'address3') || null,
    gender: normalizeGender(get('gender', 'gender', 'sex')),
    categories: normalizeCategories(get('category', 'category', 'categories')),
  ...
  for (let i = 0; i < rows.length; i += WRITE_CONCURRENCY) {
    ...
    await Promise.all(chunk.map(async (raw) => {
      ...
      const contact = {
        ...
        category: row.categories[0] || null, // kept in sync for any old code path still reading the singular field
        notes: row.notes,
        archived: false,
        createdAt: now,
