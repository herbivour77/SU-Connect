...
exports.handler = async (event) => {
  ...
  const { userId, updates, revokeSessions } = body;
  if (!userId || (!updates && !revokeSessions)) {
    return jsonResponse(400, { error: 'userId and (updates or revokeSessions) are required' });
  }
  ...
  if (
    (target.role === 'super_admin' || (updates && updates.role === 'super_admin')) &&
  ...
  let sessionsShouldBeRevoked = !!revokeSessions;

  for (const field of allowedFields) {
    if (!updates || !(field in updates)) continue;
  ...
  // A role, status, or permission change — or an explicit request — invalidates
  // existing sessions so the new access level takes effect immediately, not
  // on next natural expiry.
  if (sessionsShouldBeRevoked) {
    ...
    await logEvent({
      ...
      actionCategory: 'auth',
      description: revokeSessions && !(updates && Object.keys(updates).length)
        ? `${actingUser.fullName} manually revoked all sessions for ${target.fullName}`
        : `Sessions for ${target.fullName} revoked following account change`,
      targetType: 'user',
      targetId: target.userId,
      success: true,
    });
