  ...
  if (importBtn && importFile) {
    ...
    importFile.addEventListener('change', async () => {
      ...
      try {
        ...
        const skipSummary = res.skipped && res.skipped.length
          ? `\n\nFirst few skipped:\n${res.skipped.slice(0, 5).map((s) => `• ${s.reason}`).join('\n')}`
          : '';
        alert(`Imported ${res.createdCount} contact(s). ${res.skippedCount} skipped.${skipSummary}`);
        renderView('contacts');
      } catch (err) {
        showMsg(err.message, true);
      }
