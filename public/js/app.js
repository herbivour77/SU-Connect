...
async function renderContacts() {
  ...
  content.innerHTML = `
    ...
    <table><thead><tr>
      ...
      <th>ID</th><th>Name</th><th>Category</th><th>Email</th><th>Phone</th><th>Actions</th>
    </tr></thead>
  ...
  const renderRows = () => {
    ...
    document.getElementById('contactsBody').innerHTML = rows.map((c) => `<tr>
      ...
      </tr>`).join('') || `<tr><td colspan="${canExport ? 7 : 6}">No matching contacts.</td></tr>`;
    wireRowButtons();
  };
...
async function renderContactActivity(contactId) {
  ...
  content.innerHTML = `
    ...
    <p class="hint-text">${esc(contact.contactId)} · ${esc(contact.email || 'no email')} · ${esc(contact.phone || 'no phone')}</p>
