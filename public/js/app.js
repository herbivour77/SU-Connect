...
async function renderContacts() {
  ...
  const categories = Array.from(new Set(contacts.flatMap((c) => c.categories || (c.category ? [c.category] : [])))).sort();
  ...
  content.innerHTML = `
    ...
    <div class="toolbar" style="align-items:flex-end;">
      <div style="flex:1;min-width:200px;"><label style="display:block;font-size:.78rem;">Search</label>
        <input id="c_q" placeholder="Search name, email, phone, address, notes…"></div>
    ...
    <table><thead><tr>
      ...
      <th>ID</th><th>Name</th><th>Gender</th><th>Categories</th><th>Email</th><th>Phone</th><th>Actions</th>
    </tr></thead>
  ...
  const renderRows = () => {
    ...
    const rows = contacts.filter((c) => {
      const cCategories = c.categories || (c.category ? [c.category] : []);
      if (status === 'active' && c.archived) return false;
      if (status === 'archived' && !c.archived) return false;
      if (cat && !cCategories.includes(cat)) return false;
      if (q) {
        const hay = [c.fullName, c.email, c.phone, c.address1, c.address2, c.address3, c.notes, c.contactId, ...cCategories].filter(Boolean).join(' ').toLowerCase();
    ...
    document.getElementById('contactsBody').innerHTML = rows.map((c) => {
      ...
      return `<tr>
        ...
        <td>${esc(c.contactId)}</td><td>${esc(c.fullName)}</td><td>${esc(c.gender)}</td><td>${esc(cCategories.join(', '))}</td>
    ...
    }).join('') || `<tr><td colspan="${canExport ? 8 : 7}">No matching contacts.</td></tr>`;
    wireRowButtons();
  };
  ...
  if (importBtn && importFile) {
    ...
    importFile.addEventListener('change', async () => {
      ...
      try {
        ...
        if (!rows.length) {
          showMsg('No rows found in that file — check the data is on the first sheet with a tab, and has a header row.', true);
          return;
        }
        showMsg(`Found ${rows.length} row(s). Confirm which columns to use…`, false);
        showImportColumnMappingModal(rows, async (columnMap) => {
          // Large files are sent in bounded chunks rather than one giant
          // request. A single Netlify Function call has a hard execution
          // timeout, so one request carrying thousands of rows risks
          // timing out partway through with no clear signal of how far it
          // got. Chunking keeps each request small and fast, and shows
          // real progress for anything big enough that "please wait"
          // would otherwise be a multi-second black box.
          ...
          for (let i = 0; i < chunks.length; i++) {
            if (chunks.length > 1) showMsg(`Importing batch ${i + 1} of ${chunks.length} (${chunks[i].length} rows)…`, false);
            const res = await Api.post('contacts-import', { rows: chunks[i], columnMap });
            createdCount += res.createdCount;
            skippedCount += res.skippedCount;
            allSkipped.push(...res.skipped);
          }

          // Use alert() here, not the inline message banner: renderView()
          // below rebuilds the whole page and wipes the banner out again
          // instantly, so a summary shown only inline was never actually
          // visible to anyone — it looked like nothing had happened even
          // when the import fully succeeded.
          const skipSummary = allSkipped.length
            ? `\n\nFirst few skipped:\n${allSkipped.slice(0, 5).map((s) => `• ${s.reason}`).join('\n')}`
            : '';
          alert(`Imported ${createdCount} contact(s). ${skippedCount} skipped.${skipSummary}`);
          renderView('contacts');
        });
      } catch (err) {
        showMsg(err.message, true);
...
//
// Scans every sheet for the first one that actually has rows, rather
// than always reading the first tab — templates with a cover/instructions
// sheet first are common, and always reading tab 1 would silently return
// nothing for those.
// Lets the person confirm which actual column in their file maps to
// each contact field, instead of the app silently guessing header names
// (which broke on real-world files — see contacts-import.js for why).
// Pre-selects a best guess per field so well-labelled files need zero
// clicks to confirm; anything unusual is still fully overridable.
const MAPPING_FIELDS = [
  { key: 'fullName', label: 'Full Name', required: true, guesses: ['full name', 'fullname', 'name'] },
  { key: 'email', label: 'Email', required: false, guesses: ['email', 'email address'] },
  { key: 'phone', label: 'Phone', required: false, guesses: ['phone', 'phone number', 'mobile'] },
  { key: 'address1', label: 'Address 1', required: false, guesses: ['address1', 'address 1', 'address', 'street address'] },
  { key: 'address2', label: 'Address 2', required: false, guesses: ['address2', 'address 2'] },
  { key: 'address3', label: 'Address 3', required: false, guesses: ['address3', 'address 3'] },
  { key: 'category', label: 'Category (comma-separate multiple in one cell)', required: false, guesses: ['category', 'categories'] },
  { key: 'gender', label: 'Gender', required: false, guesses: ['gender', 'sex'] },
  { key: 'notes', label: 'Notes', required: false, guesses: ['notes', 'note'] },
];

function showImportColumnMappingModal(rows, onConfirm) {
  const headers = Object.keys(rows[0] || {});
  const guessHeaderFor = (guesses) => headers.find((h) => guesses.includes(h.trim().toLowerCase())) || '';

  showModal(`
    <h3>Match Your Columns</h3>
    <p class="hint-text">We found these columns in your file: ${headers.map((h) => esc(h)).join(', ')}. Confirm which one is which below.</p>
    ${MAPPING_FIELDS.map((f) => {
      const guessed = guessHeaderFor(f.guesses);
      return `
        <label>${esc(f.label)}${f.required ? ' (required)' : ''}</label>
        <select class="mapField" data-field="${f.key}">
          ${!f.required ? `<option value="">— Not in this file —</option>` : ''}
          ${headers.map((h) => `<option value="${esc(h)}" ${h === guessed ? 'selected' : ''}>${esc(h)}</option>`).join('')}
        </select>`;
    }).join('')}
    <p class="hint-text">Preview of row 1: ${esc(JSON.stringify(rows[0]).slice(0, 200))}</p>
  `, async () => {
    const columnMap = {};
    document.querySelectorAll('.mapField').forEach((sel) => {
      if (sel.value) columnMap[sel.dataset.field] = sel.value;
    });
    if (!columnMap.fullName) {
      throw new Error('Please select which column holds the Full Name.');
    }
...
// Lets the person attach more than one category to a contact (e.g. a
// contact can be both "donor" and "volunteer") via chips instead of a
// single free-text/select field. Wired up right after the modal HTML
// (which must include #categoryChips, #m_categoryInput, #addCategoryBtn)
// is already in the DOM — showModal() appends synchronously before
// returning, so this is safe to call immediately after showModal(...).
function setupCategoryTagPicker(initial, allCategories) {
  let selected = Array.from(new Set((initial || []).map((c) => String(c).trim().toLowerCase()).filter(Boolean)));
  const chipsEl = document.getElementById('categoryChips');
  const inputEl = document.getElementById('m_categoryInput');
  const addBtn = document.getElementById('addCategoryBtn');

  function render() {
    chipsEl.innerHTML = selected.length
      ? selected.map((c, i) => `<span class="chip">${esc(c)} <button type="button" class="chipRemove" data-i="${i}">&times;</button></span>`).join('')
      : '<span class="hint-text">No categories yet — type one below and click Add.</span>';
    chipsEl.querySelectorAll('.chipRemove').forEach((btn) => {
      btn.addEventListener('click', () => { selected.splice(Number(btn.dataset.i), 1); render(); });
    });
  }
...
function categoryPickerHtml(allCategories) {
  return `
    <label>Categories</label>
    <div id="categoryChips" class="chip-row"></div>
    <div style="display:flex; gap:6px;">
      <input id="m_categoryInput" list="categoryOptions" placeholder="e.g. donor, volunteer, participant — or type a new one" style="flex:1;">
      <button type="button" class="secondary" id="addCategoryBtn" style="width:auto; white-space:nowrap;">Add</button>
    </div>
    <datalist id="categoryOptions">${allCategories.map((c) => `<option value="${esc(c)}">`).join('')}</datalist>
  `;
}
...
function showAddContactModal(categories = []) {
  ...
  showModal(`
    ...
    ${categoryPickerHtml(categories)}
    <label>Email</label><input id="m_email" type="email" />
    <label>Phone</label><input id="m_phone" />
    <label>Address 1</label><input id="m_address1" />
    <label>Address 2</label><input id="m_address2" />
    <label>Address 3</label><input id="m_address3" />
    <label>Notes</label><textarea id="m_notes" rows="2"></textarea>
    <div class="error-text" id="modalError" style="display:none;"></div>
  `, async () => {
    await Api.post('contacts', {
      fullName: val('m_fullName'), gender: val('m_gender'), categories: categoryPicker.get(),
      email: val('m_email'), phone: val('m_phone'),
      address1: val('m_address1'), address2: val('m_address2'), address3: val('m_address3'),
      notes: val('m_notes'),
    });
    renderView('contacts');
  });
  categoryPicker = setupCategoryTagPicker([], categories);
}

function showEditContactModal(contact, categories = []) {
  const contactCategories = contact.categories || (contact.category ? [contact.category] : []);
  let categoryPicker;
  showModal(`
    <h3>Edit Contact</h3>
    ...
    <label>Address 1</label><input id="m_address1" value="${esc(contact.address1 || '')}" />
    <label>Address 2</label><input id="m_address2" value="${esc(contact.address2 || '')}" />
    <label>Address 3</label><input id="m_address3" value="${esc(contact.address3 || '')}" />
  ...
  categoryPicker = setupCategoryTagPicker(contactCategories, categories);
}
...
function showMergeContactModal(contact, allContacts) {
  ...
  showModal(`
    ...
    <p class="hint-text">This record will be archived. Any of its fields missing on the target will be copied over, and its categories will be added to the target's (nothing is dropped). This cannot be undone, but both records remain permanently in Admin History.</p>
...
async function renderContactActivity(contactId) {
  ...
  content.innerHTML = `
    ...
    <h3 style="color: var(--su-navy); margin-top: 14px;">${esc(contact.fullName)} ${(contact.categories || (contact.category ? [contact.category] : [])).map((c) => `<span class="badge role">${esc(c)}</span>`).join(' ') || '<span class="badge role">uncategorised</span>'}</h3>
    ...
    ${[contact.address1, contact.address2, contact.address3].some(Boolean) ? `<p class="hint-text">${[contact.address1, contact.address2, contact.address3].filter(Boolean).map(esc).join(', ')}</p>` : ''}
